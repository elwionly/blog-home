<template>
  <section :id="MENULINKS[3].ref" class="section-container relative min-h-screen w-full flex flex-col select-none justify-center py-8">
    ...
    <div class="grid grid-cols-12 mt-20 gap-4">
      <div ref="svgContainer" class="line-svg col-span-12 md:col-span-6">
        <svg ref="timelineSvg" :width="svgWidth" :height="svgLength" :viewBox="`0 0 ${svgWidth} ${svgLength}`" fill="none" />
      </div>
      <div class="col-span-12 hidden md:col-span-6 md:flex">
        <div ref="screenContainer" class="h-96 max-w-full overflow-hidden rounded-2xl bg-gray-800 shadow-xl">
          <img class="h-8 w-full" src="/timeline/title-bar.svg" alt="Title bar" width="644" height="34">
          <div class="relative h-full w-full -mt-2">
            <div class="absolute left-0 top-0 h-full w-full">
              <img
                v-for="(item, idx) in svgCheckpointItems"
                :key="`${item.title}-${idx}`"
                alt="Timeline"
                layout="fill"
                :class="`w-full absolute top-0 object-cover slide-${idx + 1}`"
                :src="`${item.slideImage}`"
              >
            </div>
          </div>
...
<script lang="ts" setup name="TimelineSection">
...
function addLineSvgAnimation(timeline: GSAPTimeline, duration: number, index: number): GSAPTimeline {
  ...
  timeline.from(svgContainer.value!.querySelectorAll(`.line-${index + 1}`), { scaleY: 0, duration }, startTime)
...
function addDivergingBranchLineAnimation(timeline: GSAPTimeline, duration: number, index: number): GSAPTimeline {
  timeline
    .from(svgContainer.value!.querySelector(`.line-${index + 1}`), { scaleY: 0, duration }, `start+=${duration * index}`)
    .from(
...
function setSlidesAnimation(timeline: GSAPTimeline): void {
  svgCheckpointItems.forEach((_, index) => {
    ...
    if (index !== 0)
      timeline.fromTo(screenContainer.value!.querySelector(`.slide-${index + 1}`), { opacity: 0 }, { opacity: 1 })
...
function initScrollTrigger(): {
  ...
  const timeline = gsap.timeline({ defaults: { ease: Linear.easeNone, duration: 0.44 } }).addLabel('start')
  let duration: number
